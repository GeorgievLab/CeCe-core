...
ObjectContainer::SizeType ObjectContainer::getCountByType(StringView typeName) const noexcept
{
    SizeType res = 0ul;
...
DynamicArray<ViewPtr<Object>> ObjectContainer::getByType(StringView typeName) const noexcept
{
...
void ObjectContainer::addPending() noexcept
{
    // Move objects
    m_data.insert(
        m_data.end(),
...
#ifdef CECE_RENDER
void ObjectContainer::draw(render::Context& context)
{
...
#ifdef CECE_RENDER
void ObjectContainer::drawStoreState(const simulation::Visualization& visualization)
{
...
#ifdef CECE_RENDER
void ObjectContainer::drawSwapState()
{
    for (const auto& obj : m_data)
        obj->drawSwapState();
